        ...
        for key, answer in answerlist.items():
            if len(answer) > 0:
                for source in answer:
                    count += 1
                    print("\nA%d: It %s" % (count, convert_to_past_tense(key)))
                    print "\nSource\n", source
                ...
                if re.search("\s"+Entity_word.lower()+"\s", line, re.IGNORECASE):
                    ...
                    if question_type == 1:
                        for verb_word in VBlist:
                            ...
                            for syn_word in get_synoym_list(verb_word):
                                # all the synonyms
                    ...
                    # if the question asks for a quantity:
                        ...
                        for syn_word in get_synoym_list(VBlist[0]):
                            found = re.search(r'\s'+Entity_word.lower()+".*"+syn_word +
                                              '\s[\D]*([\d]+(\s[\d]+\/[\d]+|\.?\d+)\%?)', line, re.IGNORECASE)
                            if found:
...
def q_type(ques, passage):
    ...
    if re.match(r"^Did.*\?$", ques):
        question_type_tag = 1
    elif re.match(r"^How\smuch.*\?$", ques) or re.match(r"^What.*\?$", ques):
        question_type_tag = 2

    if question_type_tag == 0:
        print "type not explored or wrong question structure"
    else:
        process_answer(ques, Entity, Verb_list, find_answer(
            Entity, Verb_list, passage, question_type_tag))
